...
<template>
  <header class="px-4 md:px-6 py-4 flex items-center justify-between z-10 rounded-lg mx-4 xl:mx-6">
    <h1>
      <NuxtLink to="/" class="text-xl md:text-2xl font-bold flex items-center space-x-2">
    ...
    <div class="flex items-center">
      ...
      <LegoGithubStar v-slot="{ stars }" repo="zernonia/nuxt-lego"
        class="mr-5 group border bg-white hover:bg-gray-100 transition rounded-lg text-sm flex justify-center">
      ...
      <button class="block md:hidden p-1 ml-1" @click="isShowingMenu = !isShowingMenu">
        <Icon name="ph:list-fill" class="text-2xl" />
      </button>
    </div>

    <Transition enter-active-class="overflow-y-hidden duration-300 ease-out"
